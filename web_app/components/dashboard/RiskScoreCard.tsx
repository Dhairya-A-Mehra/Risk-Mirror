...
export function RiskScoreCard({ title, score, icon, color }: RiskScoreCardProps) {
  // Strict fallback: always a valid number between 0 and 100
  let safeScore = Number(score);
  if (!Number.isFinite(safeScore) || isNaN(safeScore)) safeScore = 0;
  ...
  const gaugeColor = colorMap[color] || '#3b82f6';
  return (
      ...
      <CardContent className="p-6 text-center h-full flex flex-col items-center justify-center">
        ...
        <div className="relative h-32 w-32 mb-2">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-3xl font-bold text-white">{safeScore.toFixed(1)}</span>
          </div>
        </div>
